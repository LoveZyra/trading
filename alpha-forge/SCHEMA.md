...
## 就是 `scripts/reporting/html_report.py`

报告渲染能力已**内置**在 `scripts/reporting/html_report.py`：CSS 与 JS 作为常量内联，无外部依赖。
...
> 模板源文件（`templates/report.css`、`templates/render.js`）随包附带，
> 是 `html_report.py` 内嵌 `_CSS`/`_JS` 常量的忠实镜像(同步生成、勿手改),仅作开发/预览用；运行期只依赖 `html_report.py` 一个文件。
...
即可得到稳定的版式、买卖点为核心的信息层级、以及图表（评分计 / 价格阶梯 / 买卖点图），
...
alert / levels 行的 `signal`(做多/观望/做空)不再手填,可由 `scripts.risk.levels.classify_signal` 按数据算:
```python
from scripts.risk.levels import classify_signal
...
`scripts/research/signals.py` 给单标的提供 **6 种互补判定**(各问不同问题,故结论可不同),配 report 的 `methods` 字段在**报告最底部**渲染成「方法×标的」对照表:
...
## 稳健性体检 / 下跌切片 / 成本敏感度(2026-07 · Round 9)

三个新顶层区块,把"诚实机器"从日志数字变成读者看得见的图表。**`build_research` 已自动生成
这三块并塞进它返回的 dict;渲染器会把 `research.robustness/downturn/cost_curve` 自动上提为
顶层区块**——手工构造报告时也可直接给顶层 key(顶层优先)。

```jsonc
"robustness": {          // 🧪 稳健性体检(B2) — 即 validation.selection_robustness 的原样输出
  "winner": "breakout", "n_trials": 7,
  "deflated_sharpe": 0.83,          // ≥0.95 = 扣掉多重检验后仍显著
  "pbo": 0.35,                      // <0.5 = 选择过程稳健
  "spa": { "spa_p": 0.21, "rc_p": 0.3, "best": "breakout" },   // p<0.05 = 非数据窥探
  "cpcv": { "median": 0.9, "q05": -0.4, "q95": 2.1, "frac_positive": 0.79,
            "n_paths": 28, "sharpe_paths": [ ... ] }           // 有 sharpe_paths 才画分布带状图
},
"downturn": {            // 📉 下跌切片(B3) — metrics.downturn_slices(策略收益, 买入持有收益)
  "rows": [{ "label": "基准最差季度 2025Q1", "period": "2025-01-02 ~ 2025-03-31",
             "strategy": -0.031, "benchmark": -0.183, "excess": 0.152 }],   // 小数,渲染成 %
  "note": "可选,覆盖默认说明"
},
"cost_curve": {          // 💸 成本敏感度(B4) — backtest.cost_sensitivity(df, signal)
  "rows": [{ "bps": 0, "total_return": 0.42, "sharpe": 1.31, "max_drawdown": -0.12 },
           { "bps": 10, ... }, { "bps": 30, ... }],
  "note": "可选"
}
```

- 渲染:`robustnessSection`(计量表 + CPCV 出样本夏普分布 SVG:每点一条路径、q05–q95 带、中位线、
  0 虚线)+ 一句人话判定;`downturnSection` / `costCurveSection` 为带色表格(红涨绿跌),
  cost_curve 附内嵌夏普对比条。
- 数据端:`V.selection_robustness(trial_returns_df)`、`M.downturn_slices(rets, bench_rets)`、
  `bt.cost_sensitivity(df, sig, levels=(0,10,30))` —— 全 CPU、无新依赖。
