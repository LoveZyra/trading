...
from ..core import backtest as bt, indicators as ind, validation as V
from ..research import autoresearch as AR
...
def build_research(df: pd.DataFrame, name: str = "标的", *, iterations: int = 14,
    ...
    # B3/B4: downturn slices + cost-sensitivity for the winner. html_report auto-lifts
    # these (and robustness) into their own top-level report blocks.
    downturn = M.downturn_slices(rets[winner], bh.returns)
    try:
        win_sig = REGISTRY[winner](**wp).generate_signal(df)
        cost_rows = bt.cost_sensitivity(df, win_sig)
    except Exception:  # noqa: BLE001
        cost_rows = []
    ...
    return {"title": title or f"自动研究详情 · 策略测试与选择 · {name}",
            ...
            "robustness": rob,
            "downturn": ({"rows": downturn} if downturn else None),
            "cost_curve": ({"rows": cost_rows,
                            "note": f"冠军 <b>{winner}</b> 同一信号在 0/10/30bps 单边成本下重跑。"
                                    "30bps 档夏普仍为正 ⇒ edge 扛得住真实摩擦;衰减到负 ⇒ 纸面收益只是成本假设。"}
