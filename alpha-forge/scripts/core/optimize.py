...
def grid_search(StrategyCls, df: pd.DataFrame, grid: dict[str, list],
                *, metric: str = "sharpe", lag: int = 1,
                ...
                cost_stress_bps: float = 0.0, **bt_kwargs) -> pd.DataFrame:
    ...
    cost_stress_bps > 0 adds a `stressed_sharpe` column (Sharpe after charging that many
    EXTRA bps on the realized turnover — see metrics.cost_stressed_sharpe) and, when
    metric='sharpe', sorts by it instead: the winner is then the config whose edge
    survives costs being worse than assumed, not the highest-churn curve-fit.
    """
    rows = []
    for params in _param_combos(grid):
        try:
            ...
            if cost_stress_bps > 0:
                row["stressed_sharpe"] = M.cost_stressed_sharpe(res.stats, cost_stress_bps)
            rows.append(row)
        except Exception as e:  # noqa: BLE001
            rows.append({**params, "error": str(e)})
    table = pd.DataFrame(rows)
    if cost_stress_bps > 0 and metric == "sharpe" and "stressed_sharpe" in table.columns:
        metric = "stressed_sharpe"
...
def walk_forward(StrategyCls, df: pd.DataFrame, grid: dict[str, list],
                 *, n_splits: int = 5, train_frac: float = 0.6,
                 metric: str = "sharpe", lag: int = 1,
                 commission_bps: float = 1.0, slippage_bps: float = 1.0,
                 anchored: bool = False, cost_stress_bps: float = 0.0) -> WalkForwardResult:
    ...
    cost_stress_bps : when > 0 and metric='sharpe', the per-fold param SELECTION uses
        the cost-stressed Sharpe (extra bps charged on train-window turnover) so the
        chosen config is robust to costs being worse than assumed. The OOS test windows
        are still traded and reported at the TRUE costs — the honesty of the final
        number is untouched; only the selection is made cost-aware.
    """
    n = len(df)
    ...
    for k in range(1, n_splits + 1):
        ...
        for params in _param_combos(grid):
            try:
                ...
                if cost_stress_bps > 0 and metric == "sharpe":
                    raw = M.cost_stressed_sharpe(stats, cost_stress_bps)
                else:
        ...
        pos_test = res.position.reindex(test.index).fillna(0.0)
        to_test = pos_test.diff().abs()
        if len(to_test):
            to_test.iloc[0] = abs(float(pos_test.iloc[0]))   # entry trade into the window
        oos_turnover.append(float(to_test.sum()))
    ...
    # annualized turnover of the stitched OOS path — lets callers (autoresearch) apply a
    # cost-stress penalty to the LEADERBOARD selection without re-running anything.
    oos_stats["turnover_annual"] = (float(sum(oos_turnover)) / len(oos) * M.TRADING_DAYS
                                    if len(oos) else 0.0)
