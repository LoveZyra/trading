...
def cost_sensitivity(prices, signal, levels=(0.0, 10.0, 30.0), *, lag: int = 1,
                     **bt_kwargs) -> list[dict]:
    """Re-run the SAME signal at several one-way cost levels — the report's
    cost-sensitivity block (B4). An edge that only exists at 0-2bps isn't an edge,
    it's a cost assumption. Each level is charged as `bps` per unit turnover
    (commission+slippage combined). Returns rows
    [{bps, total_return, sharpe, max_drawdown, turnover_annual}] ready for
    html_report's `cost_curve`."""
    rows = []
    for bps in levels:
        r = backtest(prices, signal, lag=lag, commission_bps=float(bps),
                     slippage_bps=0.0, **bt_kwargs)
        rows.append({"bps": float(bps),
                     "total_return": round(r.stats["total_return"], 4),
                     "sharpe": round(r.stats["sharpe"], 3),
                     "max_drawdown": round(r.stats["max_drawdown"], 4),
                     "turnover_annual": round(r.stats["turnover_annual"], 2)})
    return rows
