...
def research_single(df: pd.DataFrame, iterations: int = 30, *, depth: str = None, seed: int = 0,
                    ...
                    slippage_bps: float = 1.0, cost_stress_bps: float = 10.0) -> ResearchReport:
    ...
    cost_stress_bps (default 10): a search this systematic will otherwise gravitate to
    high-turnover configs whose paper edge dies at real-world costs. The stress only
    affects walk-forward's per-fold param selection — OOS numbers stay at true costs.
    Set 0 to disable.
    """
    ...
    for _ in range(iterations):
        ...
        try:
            ...
            sel = (M.cost_stressed_sharpe(wf.oos_stats, cost_stress_bps)
                   if cost_stress_bps > 0 else sh)
        except Exception:  # noqa: BLE001
            log.debug("trial failed: %s %s", fam, params, exc_info=True)
            sh, ret, sel = float("nan"), float("nan"), float("nan")
        bandit.update(fam, _reward(sel))
        trials.append(Trial(fam, params, sh, ret, {"sel_score": sel} if cost_stress_bps > 0 else {}))
...
def research_portfolio(data: dict, iterations: int = 24, *, seed: int = 0,
                       ...
                       use_ml: bool = True, oos_frac: float = 0.3,
                       cost_stress_bps: float = 10.0) -> ResearchReport:
    ...
    cost_stress_bps (default 10): selection (bandit reward + leaderboard winner) uses the
    cost-STRESSED Sharpe — the Sharpe left after charging that many extra bps on the
    trial's realized turnover — so the search stops favoring high-churn blends whose
    edge dies at real costs. Displayed sharpes stay at true costs. Set 0 to disable.
    """
    rng = random.Random(seed)
    ...
    for _ in range(iterations):
        ...
        if d == "factor":
            ...
            try:
                ...
                sel = M.cost_stressed_sharpe(stats, cost_stress_bps) if cost_stress_bps > 0 else sh
            except Exception:  # noqa: BLE001
                log.debug("factor trial failed: %s", wd, exc_info=True)
                sh, ret, sel = float("nan"), float("nan"), float("nan")
            trials.append(Trial("factor", wd, sh, ret,
                                {"sel_score": sel} if cost_stress_bps > 0 else {}))
            bandit.update(d, _reward(sel))
        else:
            ...
            try:
                ...
                sh, ret, ic, sel = float("nan"), float("nan"), float("nan"), float("nan")  # lib missing -> skip
            except Exception:  # noqa: BLE001
                log.debug("model trial failed: %s", md, exc_info=True)
                sh, ret, ic, sel = float("nan"), float("nan"), float("nan"), float("nan")
            extra = {"ic": ic}
            if cost_stress_bps > 0:
                extra["sel_score"] = sel
            trials.append(Trial(f"model:{md}", {"model": md}, sh, ret, extra))
            bandit.update(d, _reward(sel))
...
def _finalize(trials: list, bandit: UCB1) -> ResearchReport:
    ...
    # Selection honors the cost-stressed score when trials carry one (sel_score);
    # the leaderboard still DISPLAYS the raw OOS sharpe at true costs.
    def _sel(t: Trial) -> float:
        s = t.extra.get("sel_score", float("nan"))
        return s if np.isfinite(s) else t.oos_sharpe
    sort_key = "sel_score" if (len(lb) and "sel_score" in lb.columns
                               and lb["sel_score"].notna().any()) else "oos_sharpe"
    if len(lb):
        lb = lb.sort_values(sort_key, ascending=False, na_position="last").reset_index(drop=True)
    valid = [t for t in trials if np.isfinite(_sel(t))]
    best = max(valid, key=_sel) if valid else trials[0]
...
# ----------------------------------------------------------------------------
def _ewma_softmax_weights(member_rets: pd.DataFrame, halflife: int = 63,
                          temp: float = 1.0) -> pd.DataFrame:
    """Causal time-varying member weights: softmax over each member's trailing EWMA
    Sharpe (shifted one bar). Members that have been working lately get more weight;
    a member that decays loses it — AlphaForge's (2406.18394) dynamic combiner idea,
    CPU-light. Falls back to equal weight until enough history accumulates."""
    mu = member_rets.ewm(halflife=halflife, min_periods=10).mean()
    sd = member_rets.ewm(halflife=halflife, min_periods=10).std()
    sh = (mu / sd.replace(0.0, np.nan)) * np.sqrt(252)
    # A flat member (zero-vol window -> NaN sharpe) gets a NEUTRAL score of 0, inside the
    # softmax. Filling AFTER normalization handed NaN members a bonus 1/n on top of rows
    # that already summed to 1 -> hidden leverage (P1 fix, 2026-07-02).
    z = np.exp((sh.shift(1).fillna(0.0) / max(temp, 1e-6)).clip(-10, 10))
    w = z.div(z.sum(axis=1), axis=0)
    return w.fillna(1.0 / max(member_rets.shape[1], 1))


def ensemble_top_k(report: ResearchReport, df: pd.DataFrame, k: int = 3, *,
                   weighting: str = "ewma", halflife: int = 63,
                   dedupe_families: bool = True,
                   commission_bps: float = 1.0, slippage_bps: float = 1.0):
    """Blend the top-k rule-strategy configs from a research_single report into one
    ensemble signal and backtest it on `df`.
    ...
    and lower overfitting risk (a free lunch when the strategies disagree). By default
    the members are deduped by family — two MA-crossover configs are nearly the same
    trade, so stacking them adds concentration, not diversification.

    weighting:
      * 'equal'  — plain average of target positions (the old behavior).
      * 'ewma'   — time-varying weights ∝ softmax of each member's trailing EWMA Sharpe
                   (causal, shifted): recent performance earns weight, decay loses it.
      * 'regime' — regime-CONDITIONAL weights (regime.regime_conditional_weights): each
                   member is weighted by how it performed in past bars of the SAME
                   vol/trend state, so the blend rotates toward defensive members when
                   the market flips risk-off — the down-leg failure fix.

    Returns (BacktestResult, members). Note: this re-backtests the blend on the `df`
    you pass. For a clean OOS read, pass a hold-out slice not used when the members
    were selected.
    """
    members, seen_fams = [], set()
    for t in report.leaderboard.itertuples():
        if np.isfinite(getattr(t, "oos_sharpe", float("nan"))) and t.direction in REGISTRY:
            if dedupe_families and t.direction in seen_fams:
                continue
            members.append((t.direction, t.params))
            seen_fams.add(t.direction)
        if len(members) >= k:
            break
    if not members:
    ...
    for fam, params in members:
        try:
            sig = REGISTRY[fam](**params).generate_signal(df).reindex(df.index).fillna(0.0)
            r = bt.backtest(df, sig, commission_bps=commission_bps,
                            slippage_bps=slippage_bps).returns
            sigs.append(sig); rets.append(r); used.append((fam, params))
        except Exception:  # noqa: BLE001
            continue
    if not sigs:
        raise ValueError("could not build any ensemble signals")

    sig_df = pd.concat(sigs, axis=1); sig_df.columns = range(len(sigs))
    ret_df = pd.concat(rets, axis=1).fillna(0.0); ret_df.columns = range(len(rets))
    if weighting == "ewma" and len(sigs) > 1:
        w = _ewma_softmax_weights(ret_df, halflife=halflife)
    elif weighting == "regime" and len(sigs) > 1:
        from ..risk import regime as RG
        w = RG.regime_conditional_weights(ret_df, df["close"], halflife=halflife)
    else:
        w = pd.DataFrame(1.0 / len(sigs), index=sig_df.index, columns=sig_df.columns)
    blended = (sig_df * w.reindex(sig_df.index).fillna(1.0 / len(sigs))).sum(axis=1)
